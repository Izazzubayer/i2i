...
Processed Images:
${batch.images.map((imgi) => `${i + 1}. ${img.originalName} - Status: ${img.status}`).join('\n')}
      `)

      // In a real appyou would fetch and add actual images
      // For nowwe'll create a manifest
  ...
  const handleConnectDAM = async (configmConfig) => {
    if (!batch) return
